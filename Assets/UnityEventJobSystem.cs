	...
	where T_Event : unmanaged
{
	...
	public void Subscribe(EventEntity entity, T_Job job, Action<T_Job> callback)
	{
#if !DISABLE_EVENT_SAFETY_CHKS
		if (_entityCallbackToIndex.ContainsKey(new EntityCallbackId<T_Job>(entity, callback)))
		{
		...
		_entityCallbackToIndex.Add(new EntityCallbackId<T_Job>(entity, callback), _subscribers.Length);
		_subscribers.Add(new Subscription(entity, job));
		_subscriberCallbacks.Add(callback);
	}

	public void Unsubscribe(EventEntity entity, Action<T_Job> callback)
	{
		EntityCallbackId<T_Job> callbackId = new EntityCallbackId<T_Job>(entity, callback);
	...
	public void QueueEvent(EventEntity entity, T_Event ev)
	{
		...
		{
			_cachedCurEvents[newEv] = _queuedEvents.Length - 1;
	...
	public void VerifyNoSubscribers()
	{
		...
		for (int i = 0; i < count; i++)
		{
			Action<T_Job> callback = _subscriberCallbacks[i];
